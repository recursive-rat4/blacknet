...
use crate::circuit::uint::UInt64;
use crate::gf2::GF2;
use core::marker::PhantomData;

pub struct Blake2b<A: UnitalAlgebra<GF2>> {
    phantom: PhantomData<A>,
}

impl<'a, A: UnitalAlgebra<GF2> + Clone + Eq> Blake2b<A>
where
    for<'b> &'b A: AlgebraOps<GF2, A>,
{
    fn g(
        state: &mut [UInt64<'a, A>; 16],
        input: &[UInt64<'a, A>; 16],
        a: usize,
        b: usize,
        c: usize,
    ...
    pub fn compress(
        ...
        for i in 0..8 {
            output[i] = output[i].bitxor(&state[i]).bitxor(&state[i + 8]);
        }
    }
}
