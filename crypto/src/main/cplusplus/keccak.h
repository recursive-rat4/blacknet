#ifndef BLACKNET_CRYPTO_KECCAK_H
#define BLACKNET_CRYPTO_KECCAK_H
...
inline constexpr std::uint64_t read_lane( std::byte const (&state)[ 200 ], int x, int y )
{
    return compat::byte::read<std::uint64_t,std::endian::little>( state + ( x + 5 * y ) * 8 );
}

inline constexpr void write_lane( std::byte (&state)[ 200 ], int x, int y, std::uint64_t v )
{
    compat::byte::write<std::uint64_t,std::endian::little>( state + ( x + 5 * y ) * 8, v );
}

inline constexpr void xor_lane( std::byte (&state)[ 200 ], int x, int y, std::uint64_t v )
{
    write_lane( state, x, y, read_lane( state, x, y ) ^ v );
}

inline constexpr void rho_and_pi_round( std::byte (&state)[ 200 ], std::uint64_t& lane, int rot, int x, int y )
{
    std::uint64_t temp = read_lane( state, x, y );
    write_lane( state, x, y, std::rotl( lane, rot ) );
    lane = temp;
}

inline constexpr void keccak_round( std::byte (&state)[ 200 ] )
{
    {
        // theta
...
inline constexpr void keccak_permute( std::byte (&state)[ 200 ] )
{
    for( int i = 0; i < 24; ++i )
    {
        keccak_round( state );
