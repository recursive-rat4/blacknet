#ifndef BLACKNET_CRYPTO_SHA2_H
#define BLACKNET_CRYPTO_SHA2_H
...
{
    constexpr static std::uint32_t Sigma0( std::uint32_t x ) noexcept
    {
        return std::rotr( x, 2 ) ^ std::rotr( x, 13 ) ^ std::rotr( x, 22 );
    }

    constexpr static std::uint32_t Sigma1( std::uint32_t x ) noexcept
    {
    ...
    constexpr static std::uint32_t Ch( std::uint32_t x, std::uint32_t y, std::uint32_t z ) noexcept
    {
        return ( x & y ) ^ ( ~x & z );
    }

    constexpr static std::uint32_t Maj( std::uint32_t x, std::uint32_t y, std::uint32_t z ) noexcept
    {
        return ( x & y ) ^ ( x & z ) ^ ( y & z );
    }

    constexpr static std::uint32_t W( std::uint32_t w[], int t )
    {
        return w[ t ] = ( sigma1( w[ t - 2 ] ) + w[ t - 7] + sigma0( w[ t - 15 ] ) + w[ t - 16 ] );
    }

    inline constexpr static void R1( std::uint32_t a, std::uint32_t b, std::uint32_t c, std::uint32_t& d,
                                                             std::uint32_t e, std::uint32_t f, std::uint32_t g, std::uint32_t& h,
                                                             std::byte const block[ 64 ], std::uint32_t const* K, std::uint32_t w[], int t )
    {
        w[ t ] = compat::byte::read<std::uint32_t,std::endian::big>( block + t * 4);
    ...
    inline constexpr static void R2( std::uint32_t a, std::uint32_t b, std::uint32_t c, std::uint32_t& d,
                                                            std::uint32_t e, std::uint32_t f, std::uint32_t g, std::uint32_t& h,
                                                            std::byte const block[ 64 ], std::uint32_t const* K, std::uint32_t w[], int t )
    {
        (void)block;
    ...
    constexpr static void transform( std::byte const block[ 64 ], std::uint32_t state[ 8 ] )
    {
...
{
    inline constexpr static std::uint64_t Ch( std::uint64_t x, std::uint64_t y, std::uint64_t z ) noexcept
    {
        return ( x & y ) ^ ( ~x & z );
    }

    inline constexpr static std::uint64_t Maj( std::uint64_t x, std::uint64_t y, std::uint64_t z ) noexcept
    {
        return ( x & y ) ^ ( x & z ) ^ ( y & z );
    }

    inline constexpr static std::uint64_t Sigma0( std::uint64_t x ) noexcept
    {
        return std::rotr( x, 28 ) ^ std::rotr( x, 34 ) ^ std::rotr( x, 39 );
    }

    inline constexpr static std::uint64_t Sigma1( std::uint64_t x ) noexcept
    {
    ...
    inline constexpr static std::uint64_t W( std::uint64_t w[], int t )
    {
        return w[ t ] = ( sigma1( w[ t - 2 ] ) + w[ t - 7] + sigma0( w[ t - 15 ] ) + w[ t - 16 ] );
    }

    inline constexpr static void R1( std::uint64_t a, std::uint64_t b, std::uint64_t c, std::uint64_t& d,
                                                            std::uint64_t e, std::uint64_t f, std::uint64_t g, std::uint64_t& h,
                                                            std::byte const block[ 64 ], std::uint64_t const* K, std::uint64_t w[], int t )
    {
        w[ t ] = compat::byte::read<std::uint64_t,std::endian::big>( block + t * 8 );
    ...
    inline constexpr static void R2( std::uint64_t a, std::uint64_t b, std::uint64_t c, std::uint64_t& d,
                                                            std::uint64_t e, std::uint64_t f, std::uint64_t g, std::uint64_t& h,
                                                            std::byte const block[ 64 ], std::uint64_t const* K, std::uint64_t w[], int t )
    {
        (void)block;
    ...
    constexpr static void transform( std::byte const block[ 128 ], std::uint64_t state[ 8 ] )
    {
...
class sha2_256 : wtf::sha2_256_base
{
private:
...
public:
    ...
    {
        std::byte bits[ 8 ] = {};
        compat::byte::write<std::uint64_t,std::endian::big>( bits, n_ * 8 );
        ...
        for( int i = 0; i < 8; ++i )
        {
            compat::byte::write<std::uint32_t,std::endian::big>( &digest[ i * 4 ], state_[ i ] );
        }
...
class sha2_224 : wtf::sha2_256_base
{
private:
...
class sha2_512 : wtf::sha2_512_base
{
private:
...
public:
    ...
    {
        std::byte bits[ 16 ] = {};
        compat::byte::write<std::uint64_t,std::endian::big>( bits + 8, n_ * 8 );

        std::size_t k = m_ < 112 ? 112 - m_ : 128 + 112 - m_;
        std::byte padding[ 128 ] = { std::byte{0x80}, };
        ...
        for( int i = 0; i < 8; ++i )
        {
            compat::byte::write<std::uint64_t,std::endian::big>( &digest[ i * 8 ], state_[ i ] );
        }
...
class sha2_384 : wtf::sha2_512_base
{
private:
...
class sha2_512_224 : wtf::sha2_512_base
{
private:
...
public:
    ...
    constexpr result_type result()
    {
        ...
        compat::byte::write<std::uint32_t,std::endian::big>( &digest[ 3 * 8 ], state_[ 3 ] >> 32 );

        return digest;
    }
};

class sha2_512_256 : wtf::sha2_512_base
{
private:
