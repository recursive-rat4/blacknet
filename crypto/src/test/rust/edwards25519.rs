...
type D = blacknet_crypto::ed25519::Edwards25519Duif;
...
fn add_duif() {
    ...
    let fx = F::with_hex("0A167A4DF3F20A7E0F6BEE97031CFF99D7190AF79EDA56E9FCE1EE0B3E53EABA");
    let fy = F::with_hex("62E44D27633DD1CB2386748EDE1BA66240A7F428F98B2BE674381AAE4699E851");
    let ad = D::new(ax, ay).unwrap();
    let bd = D::new(bx, by).unwrap();
    let cd = D::new(cx, cy).unwrap();
    let dd = D::new(dx, dy).unwrap();
    let fd = D::new(fx, fy).unwrap();
    let ae = E::new(ax, ay).unwrap();
    let be = E::new(bx, by).unwrap();
    let ce = E::new(cx, cy).unwrap();
    let de = E::new(dx, dy).unwrap();
    let ee = E::new(ex, ey).unwrap();
    assert_eq!(ae + bd, ce);
    assert_eq!(be + ad, ce);
    assert_eq!(de + dd, ee);
    assert_eq!(ee + fd, E::ZERO);
    assert_eq!(E::ZERO + cd, ce);
}
...
fn neg_duif() {
    ...
    let bx = F::with_hex("536670A3290765ACDCDBBDC7234F1EDD0C3B7496F2E876A29B39DD0184ECB77A");
    let by = F::with_hex("3B94477B1521DE0931F76F6CEE88E34BC4E4B581F88D8EAE8616625EC8046C4F");
    let a = D::new(ax, ay).unwrap();
    let b = D::new(bx, by).unwrap();
    assert_eq!(-a, b);
}
