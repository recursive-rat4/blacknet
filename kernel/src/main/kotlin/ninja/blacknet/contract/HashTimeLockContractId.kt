...
/**
 * An id of a hash time lock contract.
 */
@Serializable(HashTimeLockContractId.Companion::class)
class HashTimeLockContractId(
    val bytes: ByteArray
) : Comparable<HashTimeLockContractId> {
    override fun equals(other: Any?): Boolean {
        return (other is HashTimeLockContractId) && bytes.contentEquals(other.bytes)
    }
    ...
    override fun toString(): String {
        return Address.encode(Address.HTLC, bytes)
    }

    override fun compareTo(other: HashTimeLockContractId): Int {
        return Arrays.compareUnsigned(bytes, other.bytes)
    }

    companion object : KSerializer<HashTimeLockContractId> {
        /**
        ...
        override fun deserialize(decoder: Decoder) = HashTimeLockContractId(
            when (decoder) {
                ...
                is JsonDecoder,
                is RequestDecoder,
                    -> Address.decode(Address.HTLC, decoder.decodeString())
                else
        ...
        override fun serialize(encoder: Encoder, value: HashTimeLockContractId) {
            when (encoder) {
                ...
                is JsonEncoder,
                    -> encoder.encodeString(Address.encode(Address.HTLC, value.bytes))
                else
