...
$(document).ready(function () {
    ...
    function staking_click() {

        mask.show();

        type = this.innerText == 'Start Staking' ? 'startstaking' : 'stopstaking';
    ...
    async function refreshStaking() {
        ...
        let btn = $('#toggle_staking'), weight = $('#staking_weight'), nethash = $('#network_weight');
        let staking = await Blacknet.getPromise('/staking/' + localStorage.account);
        if (staking.hashRate > 0){
            weight.text(staking.weight/1e8 + ' BLN');
            btn.text('Stop Staking');
        }else{
            weight.text((0).toFixed(8) + ' BLN');
            btn.text('Start Staking');
        }

        nethash.text(staking.networkWeight/1e8 + ' BLN');
    ...
    body.on("click", "#toggle_staking", staking_click)
